        ...
        <div class="menu">
          <div>
            ...
            <p
              ...
              v-show="item.index!==1||loginState"
              :key="item.icon"
          ...
          <div>
            ...
            <p
              ...
              v-show="(item.index!==2&&item.index!==4)||loginState"
              :key="item.icon"
              :class="currPath===item.path ? 'item-click' : 'item'"
              @click="pageChange(item.index)"
            >
          ...
          <div v-if="loginState">
            <div class="group">
              <span>
                创建的歌单
                <svg-icon :name="isShowCreate?'down':'right'" style="font-size: 12px;cursor: pointer; margin-left: 2px" @click="isShowCreate=!isShowCreate" />
              </span>
              <div>
                <PlusSquareOutlined class="discolour" style="margin-right: 2px" />
              </div>
            </div>
          ...
          <div v-if="loginState" style="margin-top: 25px">
            <div class="group">
              <span>
                收藏的歌单
                <svg-icon :name="isShowCollect?'down':'right'" style="font-size: 12px;cursor: pointer ; margin-left: 2px" @click="isShowCollect=!isShowCollect" />
              </span>
            </div>
    ...
    const refreshLoginState = () => {
      getAccountInfo().then((res:any) => {
        if (res.code === 200) {
          if (!res.account) {
            logout()
            if (store.state.showApp) {
              message.error('未登录，扫码登录体验更多精彩！')
            }
          } else {
            if (localStorage.getItem('nickName') !== '未登录') {
...
.layout .header {
  ...
  padding: 10px 20px;
  user-select: none;
  box-shadow: rgb(16 16 16 / 15%) 0px 0px 4px;
}
...
.sider .logo:hover {
  transition-timing-function: cubic-bezier(0.6, 2.29, 0.42, 0.37);
  transform: scale(1.1);
}
